...
fn parse_inside_attribute_block(inside_attr_block: &str) -> Option<HeadingAttributes<'_>> {
    ...
    let mut remaining = inside_attr_block;
    while !remaining
        .trim_start_matches(|ch: char| ch.is_ascii_whitespace())
        .is_empty()
    {
        let (attr, rest) = take_heading_attribute(remaining);
        remaining = rest;
...
fn take_heading_attribute(input: &str) -> (&str, &str) {
    ...
    if let Some(equals) = input[..bare_end].find('=') {
        if let Some(&quote) = input.as_bytes().get(equals + 1) {
            if matches!(quote, b'\'' | b'"') {
                let value_start = equals + 2;
                if let Some(close) = input[value_start..].find(char::from(quote)) {
                    let quoted_end = close + value_start + 1;
                    if input
                        .as_bytes()
                        .get(quoted_end)
